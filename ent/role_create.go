...
func (rc *RoleCreate) defaults() error {
	if _, ok := rc.mutation.DeleteTime(); !ok {
		v := role.DefaultDeleteTime
		rc.mutation.SetDeleteTime(v)
	}
...
func (rc *RoleCreate) check() error {
	if _, ok := rc.mutation.DeleteTime(); !ok {
		return &ValidationError{Name: "delete_time", err: errors.New(`ent: missing required field "Role.delete_time"`)}
	}
	if _, ok := rc.mutation.CreateTime(); !ok {
